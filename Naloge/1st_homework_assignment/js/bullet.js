            ...
            if (walls[i].status) {
              if (
                (this.x + this.width > walls[i].x &&
                  this.x + this.width < walls[i].x + walls[i].width &&
        ...
        if (boss.status) {
          ...
          if (
            (this.y < boss.y + boss.height &&
              ...
              this.x + this.width < boss.x + boss.width &&
              this.y > boss.y) ||
            (this.y < boss.y + boss.height &&
        ...
        for (var i = 0; i < enemies.length; i++) {
          for (var j = 0; j < enemies[i].length; j++) {
            if (enemies[i][j].status) {
              if (
                (this.y < enemies[i][j].y + enemies[i][j].height &&
                  this.x + this.width > enemies[i][j].x &&
                  this.x + this.width < enemies[i][j].x + enemies[i][j].width &&
                  this.y > enemies[i][j].y) ||
                (this.y < enemies[i][j].y + enemies[i][j].height &&
                  this.x < enemies[i][j].x + enemies[i][j].width &&
                  this.x > enemies[i][j].x &&
                  this.y > enemies[i][j].y)
              ) {
                enemies[i][j].lives--;
            ...
            if (walls[i].status) {
              if (
                (this.y < walls[i].y + walls[i].height &&
                  this.x + this.width > walls[i].x &&
                  this.x + this.width < this.x + this.width) || //POPRAVI!!!!!
                (this.y < walls[i].y + walls[i].height &&
                  this.x < walls[i].x + walls[i].width &&
                  this.x > walls[i].x)
              ) {
