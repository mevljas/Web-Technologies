let startTime = 0;
let previousEnemyX = 0; //offSet calculation
let changeDirection = false;
let alreadyMovedDown = false; //prevents multiple movements ( they'd move 4 times - 4 rows)
let enemyWidth;
let enemyHeight;
let speedX;
...
function makeEnemies() {
  enemyWidth = width / 22; //dynamic size of a enemy
  enemyHeight = height / 22;
  ...
  enemiesAlive = enemies.length * enemies[0].length; //number of enemies.
  if (level < 3) {
    //top ship / boss
...
function enemyShoot(enemy) {
  if (enemy == undefined) {
    ...
    for (let i = 0; i < enemies.length; i++) {
      for (let j = 0; j < enemies[i].length; j++) {
        // check if the player is under the current enemy.
        if (
  ...
  for (let i = 0; i < enemies.length; i++) {
    for (let j = 0; j < enemies[i].length; j++) {
      if (!enemies[i][j].status) {
        ...
        //random x
        let x = Math.random() * (width - enemyWidth - enemyWidth) + enemyWidth;
        //check offset
